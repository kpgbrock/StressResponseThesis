function [results] = rankOrderPointMutVHL()

load upVHLPointMutResults;

% Normalize the burial traces
bt = upVHLPointMutResults.bt;
N = length(upVHLPointMutResults.ptMut);
for i=1:N
    bt(:,i) = bt(:,i)/max(bt(:,i));
end

% Get actual pair swap positions before truncating
ps = [upVHLPointMutResults.ptMut(:,1) + 62, upVHLPointMutResults.ptMut(:,2)];
...
% Put rankings back in order of point mutation indices
