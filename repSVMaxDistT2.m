...
function [results] = repSVMaxDistT2(increment)
...
for i=2:n
    ...
    for j=1:(i-1)
        ...
        lb = zeros(n,1);
        [copt,V,eps,emin,r2max,msr,~,~] = pholder(mutSeq);
        Aeq = [ones(1,n); eps'];
        beq =[msr * n; emin+increment];
        btGS = V*copt;
        [coptNew,dotprod] = linprog(copt'*(V')*V ,diag(ones(n,1)),ones(n,1)*r2max,Aeq,beq,lb,[],[],options);
        btNew2 = V*coptNew;
        
        results.divScores(k) = sum(abs(btGS-btNew2));
        results.eminGS(k) = emin;
        results.dotProd(k) = dotprod;
        results.possMut(k,:) = [i,j];
        results.btGS(:,k) = btGS;
        results.btNew(:,k) = btNew2;
