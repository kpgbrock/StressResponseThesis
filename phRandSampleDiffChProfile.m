function [results] = phRandSampleDiffChProfile(nTrials,phLow,phHi)
...
results.avgSelfIntCh = mean(phycfull.charge(:,y2hdata.idxSelfInt)')';
results.avgAllCh = mean(phycfull.charge(:,:)')';
results.avgSelfIntLength = mean(phycfull.chPerLen(:,y2hdata.idxSelfInt)')';
...
for i=1:nTrials
   z = randperm(N);
   z = z(1:length(y2hdata.idxSelfInt));
